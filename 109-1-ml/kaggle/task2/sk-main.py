...
features_train = train_dataset.iloc[:, 0:16]
classes_train = train_dataset.iloc[:, 16]

features_test = test_dataset.iloc[:, 0:16]
classes_test = test_dataset.iloc[:, 16]

print(features_train[:5])
print(classes_train[:5])


# ont_hoted_classes = pd.get_dummies(classes_train)
# labels = ont_hoted_classes.columns.values
...
clf = linear_model.LogisticRegression(random_state=0, solver='lbfgs',max_iter=500000)
clf.fit(features_train, classes_train)

# clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(16, 6), random_state=0, max_iter=15000)
# clf.fit(features_train, classes_train)
...
result = clf.predict(dataset_submission.iloc[:, 0:16])
